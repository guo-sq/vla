...
@dataclasses.dataclass
class DifficultyLabelSampleWeightPreprocessor(FrameAttributeProcessor):
    """Multiply sample_weight by offline difficulty label weights.

    The label file is relative to one dataset repo root by default and is
    expected to contain one jsonl record per episode with a ``sample_weight``
    list. This processor is useful in eager mode; lazy mode can use the
    DataConfig ``difficulty_label_file`` field, which applies the same file at
    sampler-index construction time.
    """

    label_file: str = DEFAULT_DIFFICULTY_LABEL_FILE
    strict: bool = False
    ...
    def __call__(self, ctx: DatasetContext, attrs: FrameAttributes) -> None:
        ...
        difficulty_weights = load_difficulty_sample_weights(
            root=ctx.root,
            total_frames=total,
            episode_data_index=ctx.episode_data_index,
            meta_episodes=getattr(ctx.meta, "episodes", None),
        ...
        if attrs.sample_weight is None:
            attrs.sample_weight = difficulty_weights
        else:
            if len(attrs.sample_weight) != total:
                raise ValueError(f"[{ctx.repo_id}] attrs.sample_weight length {len(attrs.sample_weight)} != {total}")
            attrs.sample_weight = (attrs.sample_weight.astype(np.int32, copy=False) * difficulty_weights).astype(
                np.int32
            )

        logger.info(
            "[%s] Applied difficulty label weights: kept=%d/%d, mean_weight=%.3f",
            ctx.repo_id,
            int(np.count_nonzero(attrs.sample_weight)),
            total,
            float(attrs.sample_weight.mean()) if total else 0.0,
        )
