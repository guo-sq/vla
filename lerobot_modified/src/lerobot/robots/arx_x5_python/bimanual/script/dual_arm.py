...
import numpy as np
from lerobot.robots.arx_x5_python.bimanual.script.single_arm import SingleArm
...
class BimanualArm:
    ...
    def set_joint_positions(
        ...
        **kwargs
        ...
        if "left" in positions:
            ...
            self.left_arm.set_catch_pos(pos=positions["left"][-1])
        if "right" in positions:
            self.right_arm.set_joint_positions(
                positions["right"],
                **kwargs
            )
            self.right_arm.set_catch_pos(pos=positions["right"][-1])
    ...
    def get_joint_currents(
        ...
        """
        Get the current joint currents for the specified arm(s).
        ...
        Returns:
            Dict[str, np.ndarray]: Dictionary containing current joint currents for the specified arm(s).
                                   Shape of each arm's velocities: (num_requested_joints,)
        """
        result = {}
        if arm in ["left", "both"]:
            result["left"] = self.left_arm.get_joint_currents(
                joint_names.get("left") if joint_names else None
            )
        if arm in ["right", "both"]:
            result["right"] = self.right_arm.get_joint_currents(
                joint_names.get("right") if joint_names else None
            )
        return result
