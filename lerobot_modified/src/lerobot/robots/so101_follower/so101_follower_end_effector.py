...
class SO101FollowerEndEffector(SO101Follower):
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(action, dict):
            ...
            elif isinstance(action, dict) and any(key.endswith('.pos') for key in action.keys()):
        ...
        # print("send_action to follower end effector: ", action)
        # if isinstance(action, np.ndarray) and action.shape == (4,):
        #     delta_ee = np.array(
        #         [
        #             action[0] * self.config.end_effector_step_sizes["x"],
        #             action[1] * self.config.end_effector_step_sizes["y"],
        #             action[2] * self.config.end_effector_step_sizes["z"],
        #         ],
        #         dtype=np.float32,
        #     )
        #     action = np.append(delta_ee, action[3])
        ...
        # print("send_action to follower::current_joint_pos: ", self.current_joint_pos)
        # print("send_action to follower::current_ee_pos: ", self.current_ee_pos)
        # Set desired end-effector position by adding delta
        desired_ee_pos = np.eye(4)
        desired_ee_pos[:3, :3] = self.current_ee_pos[:3, :3]  # Keep orientation
        # print("send_action to follower::desired_ee_pos: current", desired_ee_pos)

        # Add delta to position and clip to bounds
        desired_ee_pos[:3, 3] = self.current_ee_pos[:3, 3] + action[:3]
        # print("send_action to follower::desired_ee_pos:  final: ", desired_ee_pos)
        ...
        # print("send_action to follower::desired_ee_pos : clip min max: ", desired_ee_pos)

        # Compute inverse kinematics to get joint positions
        target_joint_values_in_degrees = self.kinematics.inverse_kinematics(
            self.current_joint_pos, desired_ee_pos
        )
        # print("send_action to follower::target_joint_values_in_degrees :", target_joint_values_in_degrees)
