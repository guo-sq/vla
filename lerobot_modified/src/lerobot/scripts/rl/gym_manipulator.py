...
class RobotEnv(gym.Env):
    ...
    def step(self, action) -> tuple[dict[str, np.ndarray], float, bool, bool, dict[str, Any]]:
        ...
        # print("RobotEnv::step::action_dict: ", action_dict)
...
class BaseLeaderControlWrapper(gym.Wrapper):
    ...
    def _handle_intervention(self, action):
        ...
        # Check for joint positions order consistency
        # Ensure both leader and follower positions are ordered consistently
        leader_joint_names = list(leader_pos_dict.keys())
        follower_joint_names = list(follower_pos_dict.keys())
        
        # print(f"DEBUG: Leader joint names: {leader_joint_names}")
        # print(f"DEBUG: Follower joint names: {follower_joint_names}")
        
        # Check if joint ordering matches - this is critical for forward kinematics
        # if leader_joint_names != follower_joint_names:
        #     print(f"WARNING: Joint name ordering mismatch!")
        #     print(f"Leader order: {leader_joint_names}")
        #     print(f"Follower order: {follower_joint_names}")
        ...
        # print(f"DEBUG: Leader joint positions: {leader_pos}")
        # print(f"DEBUG: Follower joint positions: {follower_pos}")
        ...
        # print(f"DEBUG: Leader EE position: {leader_ee}")
        # print(f"DEBUG: Follower EE position: {follower_ee}")
        # print(f"DEBUG: EE position difference: {leader_ee - follower_ee}")

        action = np.clip(leader_ee - follower_ee, -self.end_effector_step_sizes, self.end_effector_step_sizes)
        # print(f"DEBUG: Clipped EE action: {action}")
        
        # Normalize the action to the range [-1, 1]
        action = action / self.end_effector_step_sizes
        # print(f"DEBUG: Normalized EE action: {action}")
    ...
    def _handle_leader_teleoperation(self):
        ...
        # print("DEBUG: prev_leader_pos_dict: ", prev_leader_pos_dict)
    ...
    def step(self, action):
        ...
        # NOTE:
        # print("robot action AFTER _handle_intervention: ", action)
        obs, reward, terminated, truncated, info = self.env.step(action)
