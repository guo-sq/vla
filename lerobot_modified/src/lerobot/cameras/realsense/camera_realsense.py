...
from ..utils import FrameRateMonitor
...
class RealSenseCamera(Camera):
    ...
    def read(self, color_mode: ColorMode | None = None, timeout_ms: int = 200) -> dict[str, np.ndarray] | np.ndarray:
        """
        Reads a single frame (color and optionally depth) synchronously from the camera.

        This is a blocking call. It waits for a coherent set of frames (color and depth if enabled)
        from the camera hardware via the RealSense pipeline.
        ...
        Returns:
            If depth is disabled: np.ndarray - The captured color frame
            If depth is enabled: dict with 'color' and 'depth' keys containing respective frames
    ...
    def _start_read_thread(self) -> None:
        ...
        # fps may have been resolved from the device profile in connect();
