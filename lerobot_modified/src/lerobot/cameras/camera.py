...
class Camera(abc.ABC):
    ...
    @abc.abstractmethod
    def async_read(self, timeout_ms: float = ..., wait_for_new: bool = True) -> np.ndarray:
        """Asynchronously capture and return a single frame from the camera.

        Args:
            timeout_ms: Maximum time to wait for a frame in milliseconds.
                        Defaults to implementation-specific timeout.
            wait_for_new: If True (default), block until a new frame arrives.
                         If False, return the latest cached frame immediately.
