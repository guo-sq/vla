...
os.environ["CUDA_VISIBLE_DEVICES"] = (
    ""  # Set to the GPU you want to use, or leave empty for CPU
)
...
min_idle_len = (
    7  # If more than this number of consecutive idle frames, filter all of them out
)
...
filter_last_n_in_ranges = (
    10  # When using a filter dict, remove this many frames from the end of each range
)
...
for ep_idx, ep in enumerate(tqdm(ds)):
    recording_folderpath = (
        ep["episode_metadata"]["recording_folderpath"].numpy().decode()
    )
    ...
    joint_velocities = [
        step["action_dict"]["joint_velocity"].numpy() for step in ep["steps"]
    ]
    ...
    is_idle_array = np.hstack(
        [
            np.array([False]),
            np.all(np.abs(joint_velocities[1:] - joint_velocities[:-1]) < 1e-3, axis=1),
        ]
    )
    ...
    is_idle_true_starts = np.where(is_idle_diff == 1)[
        0
    ]  # +1 transitions --> going from idle to non-idle
    is_idle_true_ends = np.where(is_idle_diff == -1)[
        0
    ]  # -1 transitions --> going from non-idle to idle
    ...
    keep_true_starts = np.where(keep_diff == 1)[
        0
    ]  # +1 transitions --> going from filter out to keep
    keep_true_ends = np.where(keep_diff == -1)[
        0
    ]  # -1 transitions --> going from keep to filter out
