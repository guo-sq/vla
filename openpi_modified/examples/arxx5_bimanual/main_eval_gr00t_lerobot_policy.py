...
class Args:
    ...
    chunk_size: int = 50
    # 一个时间300，action chunk=10 那么一个轨迹的step就是30，num_steps可以根据想画多少条轨迹 自己设定
    num_steps: int = 1500
    # TODO(heyuan): auto detect num_action_dims(get from policy)
    num_action_dims: int = 14
    ...
    env: EnvMode = EnvMode.LEROBOT
    save_path: str | None = None
    dataset_root: str | None = None
    dataset_repo_id: str | None = None


def xd(data, save_path="./14d_timeseries_plot.png", title="14-dimensional Time Series"):
    """
...
def plot_14d_vertical_layout(
    ...
    # 为每个维度绘制子图
    for dim in range(args.num_action_dims):
        row, col = dim // 7, dim % 7
...
def plot_7d_vertical_layout(
    pred_data,
    gt_data,
    save_path="./14d_vertical_comparison.png",
    title="7-dimensional Time Series: Prediction vs Ground Truth",
    args=None,
):
    """
    Plots 7 subplots in vertical layout (1 per row) to compare prediction and ground truth
    """
    # 验证输入数据
    # if pred_data.shape != (50, 14) or gt_data.shape != (50, 14):
    #     raise ValueError("Both pred_data and gt_data must have shape (50, 14)")
...
def main(args: Args) -> None:
    ...
    dataset = LeRobotDataset(
        repo_id=args.dataset_repo_id,  # 自定义名称，必须非空且符合格式
    ...
    data_name = args.dataset_root.split("/")[-1]
    ...
    episode_step = args.num_steps // chunk_size
    ...
    for id in tqdm.trange(args.num_steps, desc="Running policy"):
        ...
        print(f"---type state---:{state.shape}")
        # print(f"---original dataset keys and shape---:")data_name
        # [print(k, value.shape, value.dtype, value.min(), value.max(), value.mean()) for k, value in dataset[id_start].items() if isinstance(value, torch.Tensor) and torch.is_floating_point(value)]

        # gr00t_lerobot
        if dataset[id_start].get("observation.images.head") is not None:
            ...
            observation = {
                ...
                "observation/front_image": (255 * obs_img_head.numpy()).astype(
                    np.uint8
                ),
                "observation/wrist_image": (255 * obs_img_rt_wrist.numpy()).astype(
                    np.uint8
                ),
                "observation/wrist_image_lf": (255 * obs_img_lf_wrist.numpy()).astype(
                    np.uint8
                ),
                "prompt": prompt,
            }
            ...
            # aloha dataset
            obs_img_head = dataset[id_start]["observation.images.cam_high"]
            obs_img_rt_wrist = dataset[id_start]["observation.images.cam_right_wrist"]
            obs_img_lf_wrist = dataset[id_start]["observation.images.cam_left_wrist"]
            prompt = dataset[id_start]["task"]
            observation = {
                "state": state.numpy(),
                "images": {
                    "cam_high": (255 * obs_img_head.numpy()).astype(np.uint8),
                    # "cam_low": np.zeros((3, 480, 640), dtype=np.uint8),
                    "cam_left_wrist": (255 * obs_img_lf_wrist.numpy()).astype(np.uint8),
                    "cam_right_wrist": (255 * obs_img_rt_wrist.numpy()).astype(
                        np.uint8
                    ),
                },
                "prompt": prompt,
            }
            # print observation keys and shape
            # print(f"---transformed observation keys and shape---:")
            ...
            # test_image = observation['images']['cam_high'].transpose([1,2,0])[:,:,::-1]
            # import cv2; cv2.imwrite("cam_high.test_eval.png", test_image)
            # test_image = observation['images']['cam_left_wrist'].transpose([1,2,0])[:,:,::-1]
            # import cv2; cv2.imwrite("cam_left_wrist.test_eval.png", test_image)
            # test_image = observation['images']['cam_right_wrist'].transpose([1,2,0])[:,:,::-1]
            # import cv2; cv2.imwrite("cam_right_wrist.test_eval.png", test_image)
        ...
        for i in range(action["actions"].shape[1]):
            print(
                f"action dimension {i}: min {np.min(action['actions'][:,i])}, max {np.max(action['actions'][:,i])}, mean {np.mean(action['actions'][:,i])}"
            )
        # import pdb; pdb.set_trace()
        ...
        if id > 0 and (id + 1) % episode_step == 0:
            ...
            output_dir = f"{args.save_path}/eval_results/{data_name}"
            os.makedirs(output_dir, exist_ok=True)
            save_path = f"{output_dir}/{(id+1)//episode_step}.png"
