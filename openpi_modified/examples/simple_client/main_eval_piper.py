...
def main(args: Args) -> None:
    ...
    dataset_root = (
        "/mnt/workspace/shared/datasets/anyverse/record.piper.grpb.duck.s100.v1022"
    )
    ...
    for id in tqdm.trange(args.num_steps, desc="Running policy"):
        ...
        # front mean:0.5205235481262207 front std:0.3381563723087311
        # wrist mean:0.45681777596473694 wrist std:0.2189803123474121
        # print(f"front mean:{obs_img_head.numpy().mean()} front std:{obs_img_head.numpy().std()}")
        # print(f"wrist mean:{obs_img_rt_wrist.numpy().mean()} wrist std:{obs_img_rt_wrist.numpy().std()}")
        ...
        pred_lst.append(action["actions"][:chunk_size])
        # ---action chunk gt---
        actions_list = []
        ...
        if id > 0 and (id + 1) % episode_step == 0:
            ...
            plot_7d_vertical_layout(
                pred_final,
                gt_final,
                save_path=f"/mnt/workspace/gwl/playground/openpi_modified/examples/simple_client/evalv2_vis_rlts/evalv2_pi0_aloha_sim_lerobot_ft_duck_1022_s6000_{(id+1)//episode_step}.png",
                title="14-dimensional Time Series: Prediction vs Ground Truth",
            )
            pred_lst = []
            gt_lst = []
