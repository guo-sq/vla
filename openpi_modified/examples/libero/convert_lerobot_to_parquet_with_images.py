...
if camera_keys:
    ...
    if info_json_style_key in metadata.features:
        ...
        if meta_shape and len(meta_shape) == 3:
            ...
            logging.info(
                f"Found image shape {h,w,c} from metadata key '{info_json_style_key}'."
            )
    ...
    if not target_example_key_found and first_cam_key in metadata.features:
        ...
        if meta_shape and len(meta_shape) == 3:
            h, w, c = meta_shape
            logging.info(
                f"Found image shape {h,w,c} from fallback metadata key '{first_cam_key}'."
            )
        # Check dtype as well if shape exists
        elif (
            "dtype" in metadata.features[first_cam_key]
            and metadata.features[first_cam_key]["dtype"] == "video"
        ):
            meta_shape = metadata.features[first_cam_key].get("shape")
            if meta_shape and len(meta_shape) == 3:
                h, w, c = meta_shape
...
if h == 0 or w == 0 or c == 0:
    ...
    logging.warning(
        f"Could not determine image dimensions reliably from metadata. Using default shape: H={h}, W={w}, C={c}"
    )
else:
...
try:
    ...
    if hasattr(dummy_dataset_for_episodes, "episode_indices"):
        ...
        logging.info(
            f"Successfully retrieved episode indices via LeRobotDataset.episode_indices"
        )
    elif (
        hasattr(dummy_dataset_for_episodes, "num_episodes")
        and dummy_dataset_for_episodes.num_episodes > 0
    ):
        logging.warning(
            "Using fallback: assuming sequential episodes 0 to num_episodes-1."
        )
    ...
    logging.info(
        f"Found {len(all_episode_ids)} episodes. First few: {all_episode_ids[:10]}..."
    )
    ...
    if dummy_dataset_for_episodes is not None:
        if hasattr(dummy_dataset_for_episodes, "close"):
            try:
                dummy_dataset_for_episodes.close()
            except Exception as close_e:
                logging.warning(f"Error closing dummy dataset: {close_e}")
...
def generate_episode_data(
    ...
    target_h,
    target_w,
    target_c,  # Pass target dimensions
    action_dim,
    state_dim,
    camera_keys,
):  # Pass other needed info
    """
    ...
    """
    logging.debug(
        f"[Generator] Starting for Ep {episode_idx_to_load}, Start Global Idx: {start_global_index}"
    )
    local_dataset = None
    num_frames = 0
    ...
    try:
        logging.debug(
            f"[Generator] Initializing LeRobotDataset for episode {episode_idx_to_load}"
        )
        ...
        logging.debug(
            f"[Generator] LeRobotDataset initialized for episode {episode_idx_to_load}."
        )

        num_frames = (
            local_dataset.num_frames if hasattr(local_dataset, "num_frames") else 0
        )
        if num_frames <= 0:
            logging.warning(
                f"[Generator] Ep {episode_idx_to_load}: num_frames={num_frames}. Skipping."
            )
        else:
            logging.info(
                f"[Generator] Ep {episode_idx_to_load}: Found {num_frames} frames."
            )

    except Exception as e:
        logging.exception(
            f"[Generator] Setup failed for episode {episode_idx_to_load}: {e}"
        )
        num_frames = 0
    ...
    if local_dataset and num_frames > 0:
        for frame_idx_in_episode in range(num_frames):
            try:
                ...
                if (
                    "frame_index" not in original_item
                    or "timestamp" not in original_item
                ):
                    logging.warning(
                        f"[Generator] Ep {episode_idx_to_load}, Rel Idx {frame_idx_in_episode}: Missing frame_index/timestamp. Skipping."
                    )
                    continue

                # --- Extract Non-Image Data (Same as before) ---
                original_frame_index = original_item["frame_index"]
                if hasattr(original_frame_index, "item"):
                    original_frame_index = original_frame_index.item()
                ...
                if hasattr(state_data, "tolist"):
                    state_list = state_data.squeeze().tolist()
                elif isinstance(state_data, (list, np.ndarray)):
                    state_list = list(np.array(state_data).astype(np.float32).squeeze())
                else:
                    state_list = [0.0] * state_dim
                ...
                if hasattr(action_data, "tolist"):
                    action_list = action_data.squeeze().tolist()
                elif isinstance(action_data, (list, np.ndarray)):
                    action_list = list(
                        np.array(action_data).astype(np.float32).squeeze()
                    )
                else:
                    action_list = [0.0] * action_dim
                new_item["action"] = [np.float32(x) for x in action_list]

                # --- Process Image Data ---
                for key in camera_keys:
                    image_tensor = original_item.get(
                        key
                    )  # Expect Float32 [0, 1], C,H,W from LeRobotDataset

                    # Validate tensor
                    if (
                        not torch.is_tensor(image_tensor)
                        or image_tensor.numel() == 0
                        or len(image_tensor.shape) != 3
                    ):
                        logging.warning(
                            f"[Generator] Ep {episode_idx_to_load}, Frame {original_frame_index}, Key '{key}': Invalid/missing image tensor. Using placeholder."
                        )
                        new_item[key] = (
                            placeholder_img_np  # Use placeholder NumPy array
                        )
                        continue
                    ...
                    try:
                        ...
                        image_np_uint8 = (
                            (image_tensor.permute(1, 2, 0) * 255.0).byte().cpu().numpy()
                        )
                        ...
                        if (
                            current_h != target_h
                            or current_w != target_w
                            or current_c != target_c
                        ):
                            logging.warning(
                                f"[Generator] Ep {episode_idx_to_load}, Frame {original_frame_index}, Key '{key}': Shape mismatch ({current_h, current_w, current_c}) vs target ({target_h, target_w, target_c}). Resizing."
                            )
                            # Ensure contiguous array for cv2
                            image_np_uint8_contiguous = np.ascontiguousarray(
                                image_np_uint8
                            )
                            resized_img = cv2.resize(
                                image_np_uint8_contiguous,
                                (target_w, target_h),
                            ...
                            if len(resized_img.shape) == 2 and target_c == 1:
                                image_np_uint8 = np.expand_dims(
                                    resized_img, axis=-1
                                )  # Add channel dim
                            elif len(resized_img.shape) == 2 and target_c > 1:
                                logging.error(
                                    f"[Generator] Ep {episode_idx_to_load}, Frame {original_frame_index}, Key '{key}': Resize resulted in 2D array but target has {target_c} channels. Using placeholder."
                                )
                                ...
                                logging.error(
                                    f"[Generator] Ep {episode_idx_to_load}, Frame {original_frame_index}, Key '{key}': Resize resulted in unexpected shape {resized_img.shape}. Using placeholder."
                                )
                                image_np_uint8 = placeholder_img_np
                        ...
                        logging.exception(
                            f"[Generator] Ep {episode_idx_to_load}, Frame {original_frame_index}, Key '{key}': Error processing image: {img_proc_e}. Using placeholder."
                        )
                ...
                logging.error(
                    f"[Generator] Ep {episode_idx_to_load}: IndexError at rel idx {frame_idx_in_episode}. Stopping generation."
                )
                break
            except Exception as frame_e:
                frame_ref = (
                    f"original frame {original_item.get('frame_index', 'N/A')}"
                    if "original_item" in locals()
                    else f"rel idx {frame_idx_in_episode}"
                )
                logging.warning(
                    f"[Generator] Ep {episode_idx_to_load}, {frame_ref}: Error processing frame: {type(frame_e).__name__}. Skipping. Details: {frame_e}"
                )
                continue
    ...
    if local_dataset is not None:
        if hasattr(local_dataset, "close"):
            try:
                ...
                logging.warning(
                    f"[Generator] Error closing dataset ep {episode_idx_to_load}: {close_e}"
                )
        del local_dataset
    gc.collect()
    logging.info(
        f"[Generator] Finished Ep {episode_idx_to_load}. Yielded: {total_frames_yielded} frames."
    )
...
for episode_idx in tqdm(all_episode_ids, desc="Processing Episodes"):
    ...
    try:
        ...
        if num_frames_in_episode == 0:
            logging.warning(
                f"Episode {episode_idx}: Generated dataset is empty. Skipping save."
            )
            ...
            episode_dataset.to_parquet(
                episode_output_path, batch_size=WRITER_BATCH_SIZE
            )
            ...
            logging.info(
                f"Episode {episode_idx}: Saved {num_frames_in_episode} frames to {episode_output_path}. Global count: {global_frame_counter}"
            )

    except datasets.exceptions.DatasetGenerationError as dge:
        logging.error(
            f"Episode {episode_idx}: DatasetGenerationError: {dge}. Skipping."
        )
        skipped_episodes.append(episode_idx)
    except Exception as e:
        ...
        if episode_dataset is not None:
            try:
                ...
                logging.warning(
                    f"Episode {episode_idx}: Error cleaning cache: {cache_clean_e}"
                )
            del episode_dataset
...
for filename in files_to_copy:
    ...
    try:
        ...
        if os.path.exists(target_path):
            ...
            logging.error(
                f"Download command for '{filename}' completed without error, but file not found at {target_path}. Check permissions or download behavior."
            )

    except Exception as e:
        # Catch errors during download (e.g., file not found, network issues)
        logging.error(
            f"Failed to download '{filename}' from {repo_id}: {type(e).__name__} - {e}"
        )
...
def get_feature_info(key, default_shape, default_dtype, default_names=None):
    ...
    if (
        ...
        logging.warning(
            f"Feature '{key}': Mismatch between names length ({len(names)}) and shape ({shape[0]}). Using default names."
        )
        ...
        if "action" in key:
            names = [f"action_dim_{i}" for i in range(shape[0])]
        elif "state" in key:
            names = [f"state_dim_{i}" for i in range(shape[0])]
...
total_output_chunks = (
    math.ceil(num_processed_episodes / CHUNK_SIZE) if num_processed_episodes > 0 else 0
)
...
if processed_episodes_info:
    ...
    first_processed_ep_info = sorted(
        processed_episodes_info, key=lambda x: x["episode_id"]
    )[0]
    ...
    first_ep_path = os.path.join(
        output_data_dir, f"chunk-{first_ep_chunk_idx:03d}", first_ep_filename
    )
    ...
    if os.path.exists(first_ep_path):
        ...
        try:
            ...
            logging.info(
                f"Episode {first_ep_idx} reloaded from Parquet (first 2 rows):"
            )
            ...
            if len(reloaded_dataset_ep) > 0:
                ...
                for key in camera_keys:
                    if key in first_item_reloaded:
                        ...
                        if hasattr(
                            img_data, "size"
                        ):  # Check if it looks like a PIL Image
                            logging.info(
                                f"    Image '{key}' size (W, H): {img_data.size}, mode: {img_data.mode}"
                            )
                            # Convert to numpy to check shape if needed
                            img_np = np.array(img_data)
                            logging.info(
                                f"    Image '{key}' numpy shape (H, W, C): {img_np.shape}, dtype: {img_np.dtype}"
                            )
                        else:
                            logging.warning(
                                f"    Image '{key}' reloaded data doesn't seem to be a PIL Image."
                            )
                    else:
                        logging.warning(
                            f"Camera key '{key}' not found in reloaded item."
                        )
            else:
...
logging.info(
    "Parquet files in 'data/' now contain both non-image features and image features (using datasets.Image)."
)
