...
class MultiAnyverseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if data_config.episode_fail and data_config.dataset_length:
            assert len(data_config.dataset_length) == len(
                data_config.episode_fail
            ), f"dataset_length size: {len(data_config.dataset_length)} != episode_fail size:{len(data_config.episode_fail)}"
            assert len(data_config.episode_fail) == len(
                self.repo_ids
        ...
        logging.info(
            f"[MultiDataset] Starting to initialize {len(self.repo_ids)} datasets " f"(lazy_load={self.lazy_load})"
        )
        ...
        for idx, repo_id in enumerate(self.repo_ids):
            ...
            logging.info(
                f"[MultiDataset] Dataset {idx+1}/{len(self.repo_ids)} '{repo_id}' " f"initialized in {ds_elapsed:.2f}s"
            )

        total_elapsed = time.time() - total_start
        logging.info(f"[MultiDataset] All {len(self._datasets)} datasets initialized " f"in {total_elapsed:.2f}s")
